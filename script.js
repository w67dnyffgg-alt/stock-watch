...
function displayStats() {
  ...
  const feelingHtml = Object.keys(feelingGroups).map((feeling) => { const g = feelingGroups[feeling]; const rate = ((g.win / g.total) * 100).toFixed(1); return `<div class="feeling-row"><strong>${feeling}</strong><span>勝率 ${rate}%</span><span>${g.win}勝 ${g.total - g.win}敗</span><span class="${profitClass(g.profit)}">${profitText(g.profit)}</span></div>`; }).join("");
  statsList.innerHTML = `<div class="stats-notebook"><div class="notebook-holes" aria-hidden="true"><span></span><span></span><span></span><span></span><span></span></div><div class="notebook-title">MY INVESTMENT REPORT <span>✦</span></div><div class="profit-feature"><div><span>確定した総利益</span><strong class="${profitClass(totalProfit)}">${profitText(totalProfit)}</strong></div><div><span>購入金額に対する損益率</span><strong class="${profitClass(totalProfit)}">${totalProfit > 0 ? "+" : ""}${totalProfitRate}%</strong></div></div><div class="report-grid"><div class="report-cell"><span>勝率</span><strong>${winRate}%</strong></div><div class="report-cell"><span>平均利益</span><strong class="plus">${profitText(averageProfit)}</strong></div><div class="report-cell"><span>平均損失</span><strong class="minus">${profitText(averageLoss)}</strong></div><div class="report-cell"><span>実取引</span><strong>${trades.length}件</strong></div><div class="report-cell"><span>売却済み</span><strong>${closedTrades.length}件</strong></div><div class="report-cell"><span>WATCH</span><strong>${watches.length}件</strong></div><div class="report-cell"><span>シミュレーション</span><strong>${simulations.length}件</strong></div><div class="report-cell"><span>途中ログ</span><strong>${totalLogCount}件</strong></div></div><div class="trade-highlights"><div><span>🏆 一番勝った取引</span><strong class="plus">${bestTrade ? `${bestTrade.stockName} ${profitText(bestTrade.profit)}` : "なし"}</strong></div><div><span>🫧 一番負けた取引</span><strong class="minus">${worstTrade ? `${worstTrade.stockName} ${profitText(worstTrade.profit)}` : "なし"}</strong></div></div><section class="feeling-report"><div class="notebook-section-title"><span>🧠 感情別勝率</span><small>気持ちと結果のつながりを振り返ろう</small></div><div class="feeling-table">${feelingHtml || `<div class="feeling-empty">売却済みの実取引がまだないよ🌸</div>`}</div></section></div>`;
}
