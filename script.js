...
function displayStats() {
  ...
  const feelingHtml = Object.keys(feelingGroups).map((feeling) => { const g = feelingGroups[feeling]; const rate = ((g.win / g.total) * 100).toFixed(1); return `<div class="feeling-card"><strong>${feeling}</strong><br>勝率 ${rate}% ・ ${g.win}勝 ${g.total - g.win}敗<br><span class="${profitClass(g.profit)}">${profitText(g.profit)}</span></div>`; }).join("");
  statsList.innerHTML = `<div class="stats-dashboard"><div class="stats-hero"><div class="stats-hero-card"><span>✨ 総利益</span><strong class="${profitClass(totalProfit)}">${profitText(totalProfit)}</strong></div><div class="stats-hero-card"><span>🌸 勝率</span><strong>${winRate}%</strong></div></div><div class="stats-mini-grid"><div class="stats-mini-card"><span>平均利益</span><strong class="plus">${profitText(averageProfit)}</strong></div><div class="stats-mini-card"><span>平均損失</span><strong class="minus">${profitText(averageLoss)}</strong></div><div class="stats-mini-card"><span>実取引</span><strong>${trades.length}件</strong></div><div class="stats-mini-card"><span>売却済み</span><strong>${closedTrades.length}件</strong></div><div class="stats-mini-card"><span>WATCH</span><strong>${watches.length}件</strong></div><div class="stats-mini-card"><span>シミュレーション</span><strong>${simulations.length}件</strong></div><div class="stats-mini-card"><span>途中ログ</span><strong>${totalLogCount}件</strong></div><div class="stats-mini-card"><span>ベスト</span><strong class="plus">${bestTrade ? bestTrade.stockName : "なし"}</strong></div></div><details class="item summary-item stats-more"><summary class="summary-toggle"><span class="item-header"><span class="item-name">💎 詳しい成績を見る</span><span class="summary-profit"><span class="summary-chevron" aria-hidden="true">⌄</span></span></span></summary><div class="summary-details"><div class="stats-mini-grid"><div class="stats-mini-card"><span>一番勝った取引</span><strong class="plus">${bestTrade ? `${bestTrade.stockName} ${profitText(bestTrade.profit)}` : "なし"}</strong></div><div class="stats-mini-card"><span>一番負けた取引</span><strong class="minus">${worstTrade ? `${worstTrade.stockName} ${profitText(worstTrade.profit)}` : "なし"}</strong></div></div><div class="summary-section-title">🧠 感情別勝率</div><div class="feeling-grid">${feelingHtml || `<div class="memo">売却済みの実取引がまだないよ🌸</div>`}</div></div></details></div>`;
}
